...
`server/partners.php` levert uitsluitend zichtbare bedrijven van de actieve editie. `dist/index.php` vult twee expliciete blokken in de HTML-template, zodat zoekmachines de namen ook zonder JavaScript zien. De overige pagina blijft statisch. `dist/partners.css` en `dist/partners.js` verzorgen de compacte slider en het volledige sponsoroverzicht. In de slider komen ALLE zichtbare sponsors voorbij: vier tegelijk op desktop, twee op mobiel, iedere 4,5 seconden een volgende groep. Ook de laatste groep verschijnt en daarna begint de cyclus opnieuw. Bezoekers kunnen zelf bladeren, vegen en pauzeren. Automatisch afspelen pauzeert bij aanwijzen/focus, buiten beeld en in een verborgen tabblad; bij minder beweging is alleen handmatig bladeren actief. Zonder JavaScript blijft de strook horizontaal scrollbaar. Het volledige overzicht eronder is direct uitgeklapt en bereikbaar via de link met het totale aantal. De GitHub Pages-kopie haalt alleen deze openbare blokken op bij het hoofddomein; de slider gebruikt ook bij een oudere API de volledige openbare lijst. Geen databasegegevens, contactpersonen of privébedragen gaan mee.
